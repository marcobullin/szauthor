...
const getCooperations = authorName =>
  pipe(
    map(
      pipe(
        prop('authors'),
...
app
  ...
  .get('/autoren/:authorName', async function(req, res) {
    ...
    const hydrationData = { author, articles: articles.slice(0, 4), collaborations }

    res.setHeader('Content-Type', 'text/html')
    res.send(
      template(assets.client.js, hydrationData, renderToString(<AuthorPage {...hydrationData} />)),
    )
  })
